    ...
    scanCandles(candles) {
        ...
        if (indicators.macdHistogram > 0 && indicators.macd > indicators.macdSignal) {
            signals.push({
                ...
                weight: 0.25,
                value: Math.min(1, indicators.macdHistogram / (Math.abs(indicators.macd) + 0.001)),
            });
        }
        ...
        const sma20 = candles.prices.slice(-20).reduce((a, b) => a + b, 0) / 20;
        ...
        const aggregatedScore = signals.reduce((sum, signal) => sum + signal.weight * signal.value, 0);
        return {
            ticker: candles.ticker,
            signals,
